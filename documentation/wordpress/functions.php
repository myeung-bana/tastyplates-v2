 ...
 * Enables cross-origin requests for direct API calls from the frontend
 * This is essential when the Next.js proxy is disabled
 */
add_filter('rest_pre_serve_request', function($served, $result, $request, $server) {
    // Set CORS headers for all REST API requests
...
add_action('graphql_register_types', function() {
    ...
    foreach ($inputTypes as $inputType) {
        ...
        register_graphql_field($inputType, 'businessHours', array(
            'type' => array('list_of' => 'BusinessHoursEntryInput'),
            'description' => 'Structured business hours with day-by-day schedule',
        ));
        ...
        register_graphql_field($inputType, 'imageGallery', array(
            'type' => array('list_of' => 'String'),
            'description' => 'Array of gallery image URLs for the listing (S3 bucket URLs)',
        ));
    ...
    // Register BusinessHoursEntryInput type for mutations
    register_graphql_input_type('BusinessHoursEntryInput', array(
        'description' => 'Input for a single business hours entry',
        'fields' => array(
            'day' => array('type' => 'String'),
            'openTime' => array('type' => 'String'),
            'closeTime' => array('type' => 'String'),
    ...
    // Register BusinessHoursEntry type for queries
    register_graphql_object_type('BusinessHoursEntry', array(
        'description' => 'A single business hours entry for a day',
        'fields' => array(
            'day' => array('type' => 'String'),
            'openTime' => array('type' => 'String'),
            'closeTime' => array('type' => 'String'),
    ...
    register_graphql_field('Listing', 'businessHours', array(
        'type' => array('list_of' => 'BusinessHoursEntry'),
        'description' => 'Structured business hours with day-by-day schedule',
        'resolve' => function($post) {
            if (function_exists('get_field')) {
                $hours = get_field('business_hours', $post->ID);
    ...
    register_graphql_field('Listing', 'imageGallery', array(
        ...
        'resolve' => function($post) {
            // First try to get from post_meta (where we store URLs directly)
            $gallery_urls = get_post_meta($post->ID, 'image_gallery', true);
            
            // If we have URLs stored directly, filter and return them
            if (!empty($gallery_urls) && is_array($gallery_urls)) {
                // Filter out invalid URLs and empty values
                $valid_urls = array_filter($gallery_urls, function($url) {
...
add_action('graphql_post_object_mutation_update_additional_data', function($post_id, $input, $post_type_object, $mutation_name) {
    ...
    if (isset($input['businessHours']) && is_array($input['businessHours'])) {
        // Process and sanitize business hours entries
        $sanitized_hours = array();
        foreach ($input['businessHours'] as $entry) {
            if (is_array($entry)) {
                $sanitized_hours[] = array(
                    'day' => isset($entry['day']) ? sanitize_text_field($entry['day']) : '',
                    'openTime' => isset($entry['openTime']) ? sanitize_text_field($entry['openTime']) : '',
                    'closeTime' => isset($entry['closeTime']) ? sanitize_text_field($entry['closeTime']) : '',
                    'closed' => isset($entry['closed']) ? (bool)$entry['closed'] : false,
                );
            }
        }
        
        if (!empty($sanitized_hours)) {
            update_post_meta($post_id, 'business_hours', $sanitized_hours);
            if (function_exists('update_field')) {
                update_field('business_hours', $sanitized_hours, $post_id);
            }
        }
    }
    ...
    // Handle image gallery (S3 URLs)
    if (isset($input['imageGallery']) && is_array($input['imageGallery'])) {
        // Filter and sanitize gallery URLs
        $gallery_urls = array_filter(
            array_map('esc_url_raw', $input['imageGallery']),
            function($url) {
        ...
        // Store URLs in post_meta (not ACF, since ACF Gallery expects attachment IDs, not URLs)
        if (empty($gallery_urls)) {
            delete_post_meta($post_id, 'image_gallery');
        } else {
            update_post_meta($post_id, 'image_gallery', $gallery_urls);
        }
    ...
    if (function_exists('update_field')) {
        ...
        // businessHours is already handled above with proper sanitization
