...
export const useProfileData = (targetUserId: number): UseProfileDataReturn => {
  ...
  useEffect(() => {
    ...
    if (targetUserId && !isViewingOwnProfile) {
      // For other users, fetch their public profile data
      fetchPublicUserData();
    } else if (isViewingOwnProfile && session?.user) {
      // For own profile, fetch complete user data including profile picture
      fetchPublicUserData();
    } else {
      setUserData(null);
      setNameLoading(false);
