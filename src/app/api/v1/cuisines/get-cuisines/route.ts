...
import { GET_ALL_CUISINES } from '@/app/graphql/Taxonomies/cuisineQueries';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await hasuraQuery(GET_ALL_CUISINES, variables);
    ...
    const total = result.data?.restaurant_cuisines_aggregate?.aggregate?.count || cuisines.length;
    ...
    console.error('Get Cuisines API Error:', error);
    return NextResponse.json(
      {
        success: false,
