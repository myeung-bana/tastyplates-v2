"use client";
import { Suspense, useEffect, useState } from "react";
import Navbar from "@/components/Navbar";
import Footer from "@/components/Footer";
import { PrivacyPolicyService } from "@/services/PrivacyPolicy/privacyPolicyService";

const privacyPolicyService = new PrivacyPolicyService();

export default function PrivacyPolicy() {
  const [policy, setPolicy] = useState<{ title: string; content: string } | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    const cached = localStorage.getItem("privacyPolicy");
    const expiry = localStorage.getItem("privacyPolicyExpiry");

    if (cached && expiry && Date.now() < parseInt(expiry)) {
      setPolicy(JSON.parse(cached));
    }

    async function fetchPolicy() {
      try {
        const data = await privacyPolicyService.getPrivacyPolicy();
        const newData = { title: data.title, content: data.content };

        localStorage.setItem("privacyPolicy", JSON.stringify(newData));
        localStorage.setItem("privacyPolicyExpiry", (Date.now() + 5 * 60 * 1000).toString());

        setPolicy(newData);
      } catch (err: any) {
        setError(err.message || "Unknown error");
      }
    }

    fetchPolicy();
  }, []);

  return (
    <>
      <Suspense fallback={<div></div>}>
        <Navbar />
      </Suspense>
      <main className="min-h-screen flex flex-col justify-between bg-white gap-[12px]">
        <div className="pt-24 px-4 flex justify-center">
          <h1 className="text-[32px] font-bold text-center text-[#31343F] mb-8 max-w-xl w-full">
            {policy?.title || "Privacy Policy"}
          </h1>
        </div>

        <div className="flex-1 flex items-center justify-center px-4 pb-8">
          <div className="w-full max-w-xl">
            <div className="text-[#31343F] flex flex-col gap-3 p-0">
              {error ? (
                <div className="text-center py-8 text-red-500">{error}</div>
              ) : policy ? (
                <section>
                  <div
                    className="prose prose-xs max-w-none text-[#31343F]"
                    dangerouslySetInnerHTML={{ __html: policy.content }}
                  />
                </section>
              ) : null}
            </div>
          </div>
        </div>

        <Footer />
      </main>
    </>
  );
}
